...
static int gap_event_trampoline_(struct ble_gap_event *event, void *context) {
  return static_cast<NimbleJbdBms *>(context)->handle_gap_event_(event);
}

static void scan_timeout_trampoline_(void *context) {
  static_cast<NimbleJbdBms *>(context)->on_scan_timeout_();
}
...
void NimbleJbdBms::ensure_gap_registered_() {
...
int NimbleJbdBms::handle_gap_event_(struct ble_gap_event *event) {
  switch (event->type) {
    case BLE_GAP_EVENT_CONNECT:
      if (event->connect.status == 0) {
        ...
        this->authentication_state_ = NimbleJbdBms::AuthState::NOT_AUTHENTICATED;
        this->random_byte_ = 0;

        struct ble_gap_conn_desc desc{};
        if (ble_gap_conn_find(event->connect.conn_handle, &desc) == 0) {
          memcpy(this->peer_addr_, desc.peer_id_addr.val, 6);
        }
...
void NimbleJbdBms::on_scan_timeout_() { this->schedule_reconnect(); }
